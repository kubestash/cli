			...
			Sessions: []coreapi.Session{
				{
					...
					Repositories: []coreapi.RepositoryInfo{
						{
							Name:      fmt.Sprintf("%s-%s-%d", pvcName, "pvc-storage", time.Now().Unix()),
					...
					Addon: &coreapi.AddonInfo{
						Name: "pvc-addon", // TODO: take this name from user?
						Tasks: []coreapi.TaskReference{
							{
								Name: "pvc-backup", // TODO: take this name from user?
							},
						},
					},
				},
	...
	return &coreapi.RestoreSession{
		...
		Spec: coreapi.RestoreSessionSpec{
			...
			DataSource: &coreapi.RestoreDataSource{
				Repository: fmt.Sprintf("%s-%s-%d", pvc.Name, "pvc-storage", time.Now().Unix()),
				Snapshot:   "latest",
				EncryptionSecret: &kmapi.ObjectReference{
					Name:      opt.encryptSecret,
					Namespace: dstNamespace,
			...
			Addon: &coreapi.AddonInfo{
				...
				Tasks: []coreapi.TaskReference{
					{
						Name: "pvc-restore", // TODO: take this name from user?
						TargetVolumes: &coreapi.TargetVolumeInfo{
