...
func isCompleted(obj *unstructured.Unstructured) (bool, error) {
	...
	case "Job":
		// A Job is complete if it has a condition of type "Complete" or "Failed" with a status of "True".
		...
		for _, conditionRaw := range conditions {
			...
			if (conditionType == string(batchv1.JobComplete) || conditionType == string(batchv1.JobFailed)) && conditionStatus == string(v1.ConditionTrue) {
				return true, nil
			}
		}
		return false, nil
